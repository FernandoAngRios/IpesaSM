...
@section('title', 'Nuevo Producto — IPESA SM')
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3">
        ...
        <h2 class="font-bold text-gray-900 flex-1">Crear nuevo producto</h2>
        <a href="{{ route('empleados.products.template') }}"
           class="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-gray-700 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
            </svg>
            Plantilla Excel
        </a>
        <a href="{{ route('empleados.products.import') }}"
           class="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-700 hover:text-brand-800 px-3 py-1.5 rounded-lg border border-brand-700/30 hover:bg-brand-50 transition-colors">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <form action="{{ route('empleados.products.store') }}" method="POST" enctype="multipart/form-data"
          x-data="{
              costo: '{{ old('costo_compra') }}',
              unidad: '{{ old('unidad_compra', 'litro') }}',
              pct: '{{ old('porcentaje_ganancia') }}',
              unidadMedida: '{{ old('unit', 'litro') }}',
              ...
              presentaciones: [
                  {nombre:'1/8 litro', litros:'0.125', precio:''},
                  {nombre:'1/4 litro', litros:'0.25',  precio:''},
                  {nombre:'1/2 litro', litros:'0.5',   precio:''},
                  {nombre:'Litro',     litros:'1',     precio:''},
                  {nombre:'Galón',     litros:'4',     precio:''},
                  {nombre:'Cubeta',    litros:'19',    precio:''}
              ],
              porLitro() {
    ...
    <div class="flex flex-col lg:flex-row lg:divide-x divide-gray-100">
        ...
        <div class="flex-1 min-w-0 divide-y divide-gray-100">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                           ...
                           placeholder="Ej: VinylMax Mate Blanco">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="unit" x-model="unidadMedida" required
                            ...
                            <option value="{{ $valor }}" {{ old('unit', 'litro') === $valor ? 'selected' : '' }}>{{ $etiqueta }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="coverage" value="{{ old('coverage') }}" step="0.1" min="0"
            ...
            {{-- Costo y margen --}}
            <div class="bg-gray-50/70 px-6 py-5 space-y-4">
                ...
                <input type="hidden" name="price" :value="precioLitro() !== null ? precioLitro().toFixed(2) : ''">
            </div>
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="rounded-xl border border-gray-200 overflow-hidden">
                    <div class="bg-gray-50 px-4 py-2.5 border-b border-gray-200 flex items-center justify-between">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock inicial por almacén</p>
                        <p class="text-xs text-gray-400">Opcional — deja en blanco si no hay existencias aún</p>
                    </div>
                    <div class="divide-y divide-gray-100">
                        ...
                        <div class="flex items-center gap-3 px-4 py-2.5">
                            <div class="flex-1 flex items-center gap-2 min-w-0">
                                ...
                                <span class="text-sm font-medium text-gray-700 truncate">{{ $sucursal->nombre }}</span>
                            </div>
                            <div class="flex items-center gap-1.5 shrink-0">
                                <input type="number" name="almacenes[{{ $sucursal->id }}]"
                                       value="{{ old('almacenes.' . $sucursal->id, '') }}"
                                       :step="stepActual" min="0"
                                       class="w-28 border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-700/20 focus:border-brand-700 transition-colors text-right"
                                       :placeholder="isDecimalUnit ? '0.000' : '0'">
                                <span class="text-xs text-gray-400 font-medium" x-text="abbrActual"></span>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-700/20 focus:border-brand-700 transition-colors resize-none"
        ...
        <div class="lg:w-80 xl:w-96 px-6 py-5 flex flex-col gap-4 bg-gray-50/40">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">Precios al público por tamaño — se calculan solos, ajusta si necesitas</p>
            </div>
